...
8. **Illustration** (optional) — If the screen includes illustration elements,
   apply `illustration-style` skill for style, color rules, and character specs.
## Output
Complete screen specification with layout, hierarchy, typography, color, spacing, responsive behavior, and dark mode.
Consider following up with `/responsive-audit` to verify the design, or apply `illustration-style` for screens with illustration elements.