...
## Color System Layers
### 1. Brand Palette
Primary color with full tonal scale (50-950 or equivalent). Add secondary or accent only if the product requires it — most apps work with one accent color.
### 2. Neutral Palette
...
## Best Practices
- Use OKLCH for token values — perceptually uniform and maintainable; include Hex as fallback for tooling compatibility
...
- Include dark mode token mappings when required — but don't default to dark mode as a style choice; light and dark both require explicit design decisions